...
from pyramid_mb2_try5 import build_sfd_mobile, SFD_mobile
import numpy as np
import time
from layers import *
...
parser.add_argument('--device', 	default=1,	type=int, help='Training device')
parser.add_argument('--batch_size',     default=14,     type=int, help='Batch size for training')
parser.add_argument('--resume',         default="./weights_of_mine/try3/Mobile_pyramid_12000.pth", type=str,
                    help='Resume from checkpoint')
parser.add_argument('--start_iter',    default=10000,   type=int, help='iteration number of last breakpoint')
parser.add_argument('--num_workers',    default=0,      type=int, help='Number of workers used in dataloading')
parser.add_argument('--cuda',                   default=True,   type=str2bool,
                    help='Use cuda to train model')
parser.add_argument('--lr', '--learning-rate',  default=1e-4,   type=float,
                    help='initial learning rate')
parser.add_argument('--save_folder',            default='weights_of_mine/try5/',
...
def train():
    ...
    for iteration in range(args.start_iter+1, args.iter+1):
        ...
        loss = 0.4 * loss_l + loss_c + 0.2 * loss_l_head + 0.5 * loss_c_head
        loss.backward()
        loss = float(loss_l) + float(loss_c) + 0.5 * float(loss_l_head) + 0.5 * float(loss_c_head)
