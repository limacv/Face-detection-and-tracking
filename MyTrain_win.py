...
parser.add_argument('--resume',         default=".\\net_weight\\resnet50_pretrained.pth",   type=str, help='Resume from checkpoint')
...
parser.add_argument('--annoPath',
                    default="R:\\WorkingArea\\WorkingArea\\PyramidBox-master\\image_and_anno\\anno\\gen_anno_file",
...
parser.add_argument('--save_point', default=5000, type=int, help='save loss and .pth in every # times of iters')
args = parser.parse_args()

# default:
# batch_size=32 | num_workers=4 | start_iter=0 | cuda=True | lr=1e-3
# save_folder=weights/ | annoPath=???
...
def train():
    ...
    for iteration in range(args.start_iter+1, args.iter+1):
        ...
        if iteration % 10 == 0:
            ...
            print('iter ' + repr(iteration) + ' || Loss: %.4f ||' % (float(loss)))
