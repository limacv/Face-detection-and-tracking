...
class Mobilenetv1(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3, stride=1, padding=1, dilation=1):
        ...
        self.conv1 = nn.Conv2d(in_channel, in_channel, kernel_size=kernel_size, stride=stride,
                               padding=padding, bias=False, groups=in_channel, dilation=dilation)
...
class Mobilenetv2(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, kernel_size=3, stride=1, t=2,
                 padding=1, activation=nn.ReLU6, dilation=1, side_way=False):
        ...
        self.conv2 = nn.Conv2d(inplanes * t, inplanes * t, kernel_size=kernel_size, stride=stride,
                               padding=padding, bias=False, groups=inplanes * t, dilation=dilation)
...
class SFD_mobile(nn.Module):
    def __init__(self, phase, num_classes, size):
        ...
        self.layer1_my = nn.Sequential(
            ...
            Mobilenetv2(64, 64, kernel_size=3, stride=1, padding=1, side_way=True),
            Mobilenetv2(64, 256, kernel_size=3, stride=1, padding=1)
        )
        self.layer2_my = nn.Sequential(
            Mobilenetv2(256, 64, kernel_size=5, stride=2, padding=2),
            Mobilenetv2(64, 512, kernel_size=3, stride=1, padding=2, dilation=2)
        )
        self.layer3_my = nn.Sequential(
            Mobilenetv2(512, 256, kernel_size=5, stride=2, padding=2),
            Mobilenetv2(256, 256, kernel_size=5, stride=1, padding=2, side_way=True),
            Mobilenetv2(256, 1024, kernel_size=3, stride=1, padding=2, dilation=2)
        )
        self.layer4_my = nn.Sequential(
            Mobilenetv2(1024, 256, kernel_size=5, stride=2, padding=2),
            Mobilenetv2(256, 2048, kernel_size=3, stride=1, padding=1)
        )
        self.layer5_my = Mobilenetv2(2048, 512, kernel_size=3, stride=2)
        self.layer6_my = Mobilenetv2(512, 256, kernel_size=3, stride=2)

        self.smooth_c3_my = Mobilenetv1(256, 256, kernel_size=3, padding=1)
        self.smooth_c4_my = Mobilenetv1(512, 512, kernel_size=3, padding=1)
        self.smooth_c5_my = Mobilenetv1(1024, 1024, kernel_size=3, padding=1)
        ...
        if phase == 'test':
            ...
            self.detect = Detect(num_classes, 0, 750, 0.3, 0.3)
