...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        :param planes: 4 * planes is the output channels #
        :param stride: output size = input size / stride
        """
...
class SFD(nn.Module):
    def __init__(self, block, num_blocks, phase, num_classes, size):
        # here: block = bottleneck, num_b=[3,4,6,3], phase=test, num_c=2, size=640
        ...
        # Bottom-up layers
        # block = BottleNeck
        self.layer1 = self._make_layer(block, 64, 3, stride=1)  # 256
        self.layer2 = self._make_layer(block, 128, 4, stride=2)  # 512
        self.layer3 = self._make_layer(block, 256, 6, stride=2)  # 1024
        self.layer4 = self._make_layer(block, 512, 3, stride=2)  # 2048
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        """
        :param block: Bottleneck layer / ResNet50
        :param planes: determine output channel #
        :param num_blocks: # of ResNet50, ALSO the depth of the net_raw
        :param stride: determine shrink size
        :return:
            input channels: planes(lastlayer)*4
    ...
    def forward(self, x):
        ...
        if self.firstTime:
            ...
            for idx, f_layer in enumerate(sources):
                ...
                if idx > 1:
                    prior_body_boxes.append(self.priorbox_body(idx-2, f_layer.shape[3], f_layer.shape[2]))

            self.priors = torch.cat([p for p in prior_boxs], 0).cuda()
            self.priors_head = torch.cat([p for p in prior_head_boxes], 0).cuda()
            self.priors_body = torch.cat([p for p in prior_body_boxes], 0).cuda()
        # ######<end>
        ...
        # # get body_loc & body_conf(max-in-out)
        # for idx, (x, lo, c) in enumerate(zip(sources[2:], self.body_loc, self.body_conf)):
        #     body_loc.append(lo(x).permute(0, 2, 3, 1).contiguous())
