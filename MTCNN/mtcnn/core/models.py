...
"""
including:
    1. 3 tasks 的损失函数的计算方法
    2. PNet, RNet, ONet 的模型
    3. Residual Block && ResNet Module
"""
...
# cls for classification, box for box regression
# factor: 因数
class LossFn:
    r"""损失函数对象，可以指定三个tasks的权重(factor)，
    里面包含了分类、框选、特殊点标记三个tasks的损失函数计算方法"""
    def __init__(self, cls_factor=1, box_factor=1, landmark_factor=1):
        # factors
        self.cls_factor = cls_factor
        self.box_factor = box_factor
        self.land_factor = landmark_factor
        # create loss_func objects to **calculate loss**
        self.loss_cls = nn.BCELoss()  # binary cross entropy
        self.loss_box = nn.MSELoss()  # mean square error
        self.loss_landmark = nn.MSELoss()  # mean square error

    def cls_loss(self, gt_label, pred_label):  # gt for get, pred for predicted
...
class PNet(nn.Module):
    ...
    def forward(self, x):
        # label: detection | offset: bounding box
        x = self.pre_layer(x)
        label = torch.sigmoid(self.conv4_1(x))
        offset = self.conv4_2(x)
        # todo: add landmark if possible
...
class ONet(nn.Module):
    ...
    def forward(self, x):
        ...
        # detection & bounding box & landmark
